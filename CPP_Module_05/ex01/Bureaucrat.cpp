...
Bureaucrat::Bureaucrat(const std::string& name, int grade): _name(name), _grade(grade) {
    if (grade < 1) {
        throw GradeTooHighException("El grado del burocrata no puede estar por encima de la jerarquia. Tiene que estar entre 1 - 150");
    }
    if (grade > 150) {
        throw GradeTooLowException("El grado del burocrata no puede estar por debajo de la jerarquia. tiene que estar entre 150 - 1");
    }
